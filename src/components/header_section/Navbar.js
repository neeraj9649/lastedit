...
function Navbar() {
  ...
  return (
    <nav className='bg-white border-b-black border-b-2 border-opacity-35 '>
      <div className='flex h-14  justify-between items-center gap-4 px-6 '>
        <div className=' z-50 px-5 py-4 md:w-auto w-full flex justify-between items-center '>
          <img src={logo} className='h-24 pt-2 md:cursor-pointer'></img>
          <div className='md:hidden text-4xl' onClick={()=>(setopen(!open))}>
            <ion-icon name={`${open ? 'close' : 'menu'}`}></ion-icon>
          </div>
        </div>
        <ul className='md:flex z-50 hidden items-center gap-7 '>
