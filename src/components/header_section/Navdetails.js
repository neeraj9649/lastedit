          ...
          <div>
            ...
            <div className={`
              ...
              {value.submenu.map(subl=>(
                <div>
                  <div>
                    <h1 className=' py-2 pl-12 font-semibold md:pr-0  pr-5' onClick={()=>sublinks === subl.title ? setsublinks('') : setsublinks(subl.title)}>{subl.title}</h1>
                  </div>
                  <div className={`${sublinks ===  subl.title? 'md:hidden' : 'hidden'}`}>
                    {subl.sublinks.map(names=>(
                      <li className='py-2 pl-16'>
                        {names.name}
                      </li>
                    ))}
                  </div>
